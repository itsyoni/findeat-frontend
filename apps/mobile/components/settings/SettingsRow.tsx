...
export default function SettingsRow({ icon, title, subtitle, value, onPress, destructive }: Props) {
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.65} className="flex-row items-center px-5 py-4">
      <View className="mr-4 h-10 w-10 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-900">{icon}</View>
      <View className="flex-1">
        <Text className={`text-base ${destructive ? 'text-red-500' : 'text-black dark:text-white'}`}>{title}</Text>
      ...
      {!destructive ? <CaretRightIcon size={18} color={isDark ? '#666' : '#AAA'} /> : null}
    </TouchableOpacity>
  );
}
