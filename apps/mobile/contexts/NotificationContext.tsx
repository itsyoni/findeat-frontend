...
import { notificationUnreadQueryKey, notificationsQueryKey } from '@/hooks/useNotifications';
...
function openPushData(data?: Record<string, unknown>) {
  if (!data) return;
  if (typeof data.conversationId === 'string') router.push(`/chats/${data.conversationId}`);
  else if (typeof data.postId === 'string') router.push({ pathname: '/(posts)/[id]', params: { id: data.postId } });
  else if (typeof data.restaurantId === 'string') router.push(`/restaurants/${data.restaurantId}`);
  else if (typeof data.actorId === 'string') router.push({ pathname: '/(users)/[id]', params: { id: data.actorId } });
}
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    socket.on('notification', (item: AppNotification) => {
      ...
      queryClient.setQueryData<{ count: number }>(notificationUnreadQueryKey, (current) => ({
        count: (current?.count ?? 0) + 1,
      }));
      void queryClient.invalidateQueries({ queryKey: notificationUnreadQueryKey });
      setPopup(item);
