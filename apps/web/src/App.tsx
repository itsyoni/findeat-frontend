    ...
    <form className="profile-form card" onSubmit={save}>
      <div className="restaurant-media-editor full">
        <div className="cover-editor">
          {coverPreview ? <img src={coverPreview} alt="Restaurant cover preview" /> : <div className="media-placeholder">Add a cover photo</div>}
          <label className="media-change-button"><input type="file" accept="image/*" onChange={(event) => selectImage(event.target.files?.[0], 'cover')} /><span>{coverFile ? 'Cover selected' : 'Change cover'}</span></label>
        </div>
        <div className="logo-editor">
          <div className="logo-preview">{logoPreview ? <img src={logoPreview} alt="Restaurant logo preview" /> : <span>{restaurant.name.charAt(0).toUpperCase()}</span>}</div>
          <div><label className="media-change-button dark"><input type="file" accept="image/*" onChange={(event) => selectImage(event.target.files?.[0], 'logo')} /><span>{logoFile ? 'Logo selected' : 'Change logo'}</span></label><small>Use a square image for the best result.</small></div>
        </div>
      </div>
